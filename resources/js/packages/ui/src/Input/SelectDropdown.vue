...
function resetHightlightedItem() {
    if (filteredItems.value.length > 0) {
        ...
        if (
            ...
            highlightedItemId.value = props.getKeyFromItem(firstItem);
        }
    }
}
...
function moveHighlightUp() {
    if (highlightedItem.value) {
        ...
        if (currentHightlightedIndex === 0) {
            const lastItem = filteredItems.value[filteredItems.value.length - 1];
            if (lastItem !== undefined) {
                highlightedItemId.value = props.getKeyFromItem(lastItem);
            }
        } else {
            const prevItem = filteredItems.value[currentHightlightedIndex - 1];
            if (prevItem !== undefined) {
                highlightedItemId.value = props.getKeyFromItem(prevItem);
            }
        }
    }
}
...
function moveHighlightDown() {
    if (highlightedItem.value) {
        ...
        if (currentHightlightedIndex === filteredItems.value.length - 1) {
            ...
            const nextItem = filteredItems.value[currentHightlightedIndex + 1];
            if (nextItem !== undefined) {
                highlightedItemId.value = props.getKeyFromItem(nextItem);
            }
        }
    }
}
